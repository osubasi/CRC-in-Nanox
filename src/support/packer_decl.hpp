...
class Packer {
   ...
   class PackInfo {
      ...
      public:
         PackInfo() : _addr( nullptr ), _len ( 0 ), _count( 0 ) { }
         PackInfo( memory::Address addr, std::size_t len, std::size_t count ) : _addr( addr ), _len ( len ), _count( count ) { }
         PackInfo( PackInfo const &pack ) : _addr( pack._addr ), _len ( pack._len ), _count( pack._count ) { }
         bool operator<( PackInfo const &pack ) const;
         bool overlaps( memory::Address addr ) const;
         bool sizeMatch( std::size_t len, std::size_t count ) const;
   };

   class PackMemory {
      memory::Address _memory;
      Atomic<unsigned int> _refCounter;

      public:
         PackMemory() : _memory( nullptr ), _refCounter( 0 ) { }
         PackMemory( memory::Address mem ) : _memory( mem ), _refCounter( 1 ) { }
         PackMemory( PackMemory const &pm ) : _memory( pm._memory ), _refCounter( pm._refCounter.value() ) { }
         memory::Address getMemoryAndIncreaseReferences() { _refCounter++; return _memory; }
         unsigned int decreaseReferences() { unsigned int value = --_refCounter; return value; }
         memory::Address getMemory() const { return _memory; }
   };
   ...
   public:
      ...
      memory::Address give_pack( memory::Address addr, std::size_t len, std::size_t count );
      bool free_pack( memory::Address addr, std::size_t len, std::size_t count, memory::Address allocAddr );
