      ...
      for ( dimIdx = 0; dimIdx < this->getNumDimensions(); dimIdx += 1) {
         if ( refDims[ dimIdx ].size == thisDims[ dimIdx ].size ) {
            ...
            if ( off % elemSize[dimIdx] != 0 ) { 
               ...
               fatal("Address does not correspond with the provided "
                     "lower_bound (", thisDims[dimIdx].lower_bound, ") "
                     "for dimension #", dimIdx, ")."
                    );
            }
         } else if ( refDims[ dimIdx ].size > thisDims[ dimIdx ].size ) {
            fatal("Can't deduct dimensions of registered object. "
                  "Specified dimension ", dimIdx, " size (", thisDims[dimIdx].size, ") "
                  "does not match with registered object dimension size (", refDims[dimIdx].size, ")."
                 );
