...
SMPTransfer::SMPTransfer( DeviceOps *ops, memory::Address dst, memory::Address src, size_t len, size_t count, size_t ld, bool in ) : _ops(ops), _dst(dst), _src(src), _len(len), _count(count), _ld(ld), _in( in ) {
   ops->addOp();
}
...
void SMPTransfer::execute() {
   ...
   for ( size_t count = 0; count < _count; count += 1) {
      ...
      if (sys._watchAddr != NULL ) {
         if (sys._watchAddr >= (_dst + count *_ld ) && sys._watchAddr < (_dst + count *_ld + _len)) {
            *myThread->_file << "WATCH update: old value " << *((double *) sys._watchAddr )<< std::endl;
         }
         if (sys._watchAddr >= (_src + count *_ld ) && sys._watchAddr < (_dst + count * _ld + _len)) {
...
void SMPTransferQueue::addTransfer( DeviceOps *ops, memory::Address dst, memory::Address src, size_t len, size_t count, size_t ld, bool in ) {
   _lock.acquire();
   // NANOS_INSTRUMENT ( static InstrumentationDictionary *ID = sys.getInstrumentation()->getInstrumentationDictionary(); )
   ...
   if ( len * count > CHUNK_SIZE*2 && ( true /* FRAGMENT FLAG */ ) ) {
      if ( count == 1 ) {
         ...
         if ( len > CHUNK_SIZE*2 ) {
            for ( size_t count_idx = 0; count_idx < count; count_idx += 1 ) {
               size_t current_line_chunk = CHUNK_SIZE;
               size_t total_line = 0;
            ...
            size_t current_count_chunk = CHUNK_SIZE / len;
